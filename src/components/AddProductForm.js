...
const AddProductForm = ({ visible, onClose }) => {
  ...
  const handleCategoryChange = async (category) => {
    try {
      const response = await axios.get(
        `https://shukra-nodejs-server.onrender.com/api/v1/subcategories/subcategories/category/${category}`
      );
  ...
  const handleAddProduct = async () => {
    try {
      ...
      //   "https://shukra-nodejs-server.onrender.com/api/v1/product/addProduct",
      //   formData,
      //   {
